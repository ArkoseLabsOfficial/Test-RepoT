...
import funkin.backend.scripting.events.character.PlayAnimEvent;
...
class Character extends CNECharacter {
	public var sprite3D:AnimatedSprite3D;
	// animOffsets, curCharacter, isPlayer, debugMode REMOVED (Inherited from CNECharacter)
	...
	public function new(x:Float, y:Float, ?character:String = 'bf', ?isPlayer:Bool = false, ?isSkin:Bool = false, ?charType:String) {
		super(x, y); // Init CNECharacter (FlxSkewedSprite)
		...
		if (attemptCNELoad(curCharacter)) {
			// If XML load successful, apply basic props that Psych needs
			...
			// Position Handling (XML x/y is usually relative or absolute depending on engine)
			// We sync positionArray so Psych knows where it is
			...
			// Setup Miss Anims Flag
			if(animOffsets.exists('singLEFTmiss') || animOffsets.exists('singDOWNmiss') || animOffsets.exists('singUPmiss') || animOffsets.exists('singRIGHTmiss')) hasMissAnimations = true;
			
			recalculateDanceIdle();
			dance();
			
			// Fix flip for player if needed
			if (isPlayer) flipX = !flipX;
			
			return; // EXIT EARLY - Skip JSON Logic
		}

		// 2. FALLBACK TO PSYCH JSON LOAD
	...
	override function update(elapsed:Float) {
		...
		super.update(elapsed); // Calls CNECharacter update -> which calls scripts
	...
	override public function playAnim(AnimName:String, Force:Bool = false, Reversed:Bool = false, Frame:Int = 0){
		...
		// CNE GLOBAL OFFSET LOGIC
		var totalOffsetX:Float = 0;
		var totalOffsetY:Float = 0;

		if (animOffsets.exists(AnimName)) {
			totalOffsetX = daOffset[0];
			totalOffsetY = daOffset[1];
		}
		
		// Add Global CNE Offsets
		totalOffsetX += globalOffset.x;
		totalOffsetY += globalOffset.y;

		var offsetType = #if flxanimate isAnimateAtlas ? atlas.offset : #end offset;
		offsetType.set(totalOffsetX, totalOffsetY);
